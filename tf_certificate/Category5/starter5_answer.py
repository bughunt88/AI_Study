...
def windowed_dataset(series, window_size, batch_size, shuffle_buffer):
    series = tf.expand_dims(series, axis=-1) # 마지막에 쉐이프 1 추가 
...
def solution_model():
    ...
    # PLEASE NOTE IF YOU SEE THIS TEXT WHILE LTRAINING -- IT IS SAFE TO IGNORE
    # BaseCollectiveExecutor::StartAbort Out of range: End of sequence
    # 	 [[{{node IteratorGetNext}}]]
    #

    loss = model.evaluate(test_set, batch_size=1)
    print('loss : ', loss)

    # YOUR CODE HERE TO COMPILE AND TRAIN THE MODE
    return model
